...
def index():
    ...
    if request.method == 'POST':
        url_input = request.form.get('urls', '').strip()  # ← name="urls" に修正
        ...
        try:
            ...
            for file in mp3_files[:len(url_list)]:
                ...
                download_links.append({'name': filename, 'link': f'/download/{filename}'})
    ...
    return render_template(
        'index.html',
        message=message,
        download_links=download_links,  # ← 忘れず渡す
        zip_link=zip_link
    )
